...
/**
 * Initializes the database with stories if it's empty.
 * This function is called on app startup to ensure production databases
 * have content without requiring manual seeding.
 */
export async function initializeStoriesIfEmpty() {
  try {
    ...
    // Don't crash the app if initialization fails - just log it
    // The app can still function, and admin can manually seed if needed
  }
}
