...
async function main() {
  console.log("\n✨ SESSION 4A: AI-Original Stories Part 1 (15 stories) ✨\n");

  let successCount = 0;

  // 5 English + 5 French + 5 German = 15 stories
  console.log("🇬🇧 English originals (5)...");
  for (let i = 0; i < 5; i++) {
    console.log(`\n[${i + 1}/5] Generating English original...`);
    try {
      ...
      await db.insert(stories).values({
        ...
        imageUrl: getImageUrl(90 + successCount),
  ...
  console.log("\n🇫🇷 French originals (5)...");
  for (let i = 0; i < 5; i++) {
    console.log(`\n[${i + 1}/5] Generating French original...`);
    try {
  ...
  console.log("\n🇩🇪 German originals (5)...");
  for (let i = 0; i < 5; i++) {
    console.log(`\n[${i + 1}/5] Generating German original...`);
    try {
  ...
  console.log(`\n✨ Session 4A Complete: ${successCount}/15 AI-original stories saved ✨\n`);
}
