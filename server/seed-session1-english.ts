...
async function main() {
  console.log("\n🌙 SESSION 1: English Classics (30 stories) 🌙\n");
  ...
  console.log(`\n\n📊 Session 1 Results:`);
  console.log(`  Successful: ${processedStories.length}/${englishClassics.length}`);
  console.log(`  Failed: ${englishClassics.length - processedStories.length}`);
  ...
  if (processedStories.length > 0) {
    ...
    console.log(`  ✅ Database updated with ${processedStories.length} English stories`);
  }

  console.log("\n✨ Session 1 complete! Ready for Session 2 (French classics) ✨\n");
}
